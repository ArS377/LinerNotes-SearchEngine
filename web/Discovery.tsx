...
export function Discovery({ url, bookmarks, navigate, renderRecording }: Props) {
  ...
  return <section className="shell page-view discovery-view">
    ...
    <form className="discovery-controls" onSubmit={(event) => { event.preventDefault(); setDismissed([]); setApplied({ focus, differentArtists: different, unfamiliarArtists: unfamiliar }); setMessage(""); setShareUrl(""); }}>
      <fieldset><legend>What matters most?</legend><div className="discovery-focus">{([{ value: "balanced", title: "A bit of both", description: "Genre first, with a similar era" }, { value: "genre", title: "Genre & scene", description: "Shared catalog tags, any year" }, { value: "era", title: "Release era", description: "Prioritize nearby release years" }] as const).map((option) => <label key={option.value}><input type="radio" name="focus" value={option.value} checked={focus === option.value} onChange={() => setFocus(option.value)} /><span><strong>{option.title}</strong><span>{option.description}</span></span></label>)}</div></fieldset>
      <div className="discovery-options"><label><input type="checkbox" checked={different} onChange={(event) => setDifferent(event.target.checked)} />Different artists from the starting track</label><label><input type="checkbox" checked={unfamiliar} onChange={(event) => setUnfamiliar(event.target.checked)} />Artists outside my bookmarks</label><button className="primary-button" type="submit" disabled={query.isFetching}>{query.isFetching ? "Finding recordings…" : "Find recommendations"}</button></div>
      <p className="discovery-note">Genre tags and release dates guide these picks. Production, vocals, and energy matching aren’t available yet.</p>
    </form>
