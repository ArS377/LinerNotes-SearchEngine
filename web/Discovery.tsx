...
import { submitDiscoveryChoices, type Choices } from "./discovery-controls.js";
...
export function Discovery({ url, bookmarks, navigate, renderRecording }: Props) {
  ...
  const [applied, setApplied] = useState<Choices | null>(null);
  ...
  const endpoint = seedSlug.startsWith("mbid-") ? `/api/external-songs/${encodeURIComponent(seedSlug.slice(5))}` : seedSlug.startsWith("apple-") ? `/api/apple-songs/${encodeURIComponent(seedSlug.slice(6))}` : `/api/songs/${encodeURIComponent(seedSlug)}`;
  const song = useQuery({
    queryKey: ["song", seedSlug],
    queryFn: () => api<{ title: string; artist: string | { name: string } }>(endpoint),
  ...
  const query = useQuery({
    ...
    enabled: validSeed && !invalidTrail && applied !== null, retry: false, staleTime: 300_000,
    refetchOnWindowFocus: false, refetchOnReconnect: false
  });
  const data = applied ? query.data : undefined;
  const seed = data?.seed || (song.data ? { slug: seedSlug, title: song.data.title, artist: typeof song.data.artist === "string" ? song.data.artist : song.data.artist.name } : undefined);
  const steps = incoming?.steps || (seed ? [{ slug: seed.slug, title: seed.title, artist: seed.artist }] : []);
  const trail: Trail = { version: 1, steps, focus: applied?.focus ?? focus, differentArtists: applied?.differentArtists ?? different };
  ...
  return <section className="shell page-view discovery-view">
    ...
    <h1>{seed ? `More like ${seed.title}` : "Find similar recordings"}</h1>
    <p className="page-intro">{seed ? `Start from ${seed.artist}. Choose the connection you want to follow.` : song.isFetching ? "Loading the starting recording…" : "Choose your filters, then find recommendations."}</p>
    {!seed && song.error && <div role="alert"><p>The starting recording couldn’t load.</p><button onClick={() => { void song.refetch(); }}>Retry recording details</button></div>}
    <form className="discovery-controls" onSubmit={(event) => { event.preventDefault(); submitDiscoveryChoices(applied, { focus, differentArtists: different, unfamiliarArtists: unfamiliar }, setApplied, () => { void query.refetch(); }); setMessage(""); setShareUrl(""); }}>
    ...
    {!applied && <p>Choose your filters, then click Find recommendations.</p>}
