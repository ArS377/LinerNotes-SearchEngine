...
function Compare({ items, toggleCompare, assistantEnabled }: { items: RecordingSummary[]; assistantEnabled: boolean; toggleCompare: (item: RecordingSummary) => void }) {
  return <section className="shell page-view"><h1>Compare recordings</h1><p className="page-intro">Choose up to three recordings. Compare their details and get a short explanation of the differences.</p>{items.length < 2 && <State title="Choose at least two recordings" body="Use Compare on any search result. Your comparison tray holds up to three recordings." action={<button className="text-link" onClick={() => navigate("/")}>Find recordings →</button>} />}{items.length >= 2 && <Comparison items={items} enabled={assistantEnabled} />}<div className="compare-grid">{items.map((item) => <article key={item.slug}><button className="remove-compare icon-button" aria-label={`Remove ${item.title} by ${item.artist} from comparison`} title="Remove from comparison" onClick={() => toggleCompare(item)}><Icon name="close" size={16} /></button><div className={`compare-art${item.artworkUrl || item.thumbnailUrl ? " has-artwork" : ""}`} style={item.artworkUrl || item.thumbnailUrl ? { backgroundImage: `url(${item.artworkUrl || item.thumbnailUrl})` } : undefined} /><span className="source-badge">{item.source || "Liner Notes"}</span><h2>{item.title}</h2><p>{item.artist}</p><dl><dt>Version</dt><dd>{String(item.version || "Recording")}</dd><dt>Released</dt><dd>{String(item.releaseDate || "Unknown")}</dd><dt>Genres</dt><dd>{Array.isArray(item.genres) ? (item.genres as string[]).join(", ") : "Unknown"}</dd></dl><button className="text-link" onClick={() => navigate(`/songs/${item.slug}`)}>Open liner notes →</button></article>)}</div></section>;
}
