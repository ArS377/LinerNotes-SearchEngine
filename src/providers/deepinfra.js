...
export async function askDeepInfra(prompt, context = {}) {
  ...
  const createCompletion = async (repair = false) => {
    const response = await fetchImplementation("https://api.deepinfra.com/v1/openai/chat/completions", {
      ...
      body: JSON.stringify({
        ...
        messages: [
          ...
          ...(isComparison ? [{ role: "system", content: `${repair ? "The previous output was incomplete. " : ""}Return only a flat JSON object matching this template. Each numbered key refers to that zero-based recording in selection order. All values must be strings, no arrays or nested objects. Fill each song key with one concise explanation of its story or lyrical themes, without repeating its title or genre. Fill style and production keys with short details, not repeated introductions. Cite claims using [1] etc. State when meaning is unsupported; distinguish interpretations from confirmed statements. No extra keys or repeated summaries. Template: ${JSON.stringify(Object.assign(Object.fromEntries(context.recordings.flatMap((recording, i) => [[`song${i}`, `Meaning of ${recording.title} by ${recording.artist}, in 1-2 sentences. Use only evidence about this recording.`], [`style${i}`, `Musical style of ${recording.title} by ${recording.artist}.`], [`production${i}`, `Production of ${recording.title} by ${recording.artist}.`]])), { uncertainty: "" }))}` }] : []),
    ...
    if (!response.ok) {
      ...
      throw Object.assign(new Error(`DeepInfra request failed (${response.status}).`), {
        code: response.status === 429 || response.status >= 500 ? "PROVIDER_BUSY" : "PROVIDER_ERROR"
      });
    }
    const body = await response.json();
    const answer = body.choices?.[0]?.message?.content;
    if (typeof answer !== "string" || !answer.trim()) {
      throw Object.assign(new Error("DeepInfra returned an empty answer."), { code: "PROVIDER_BUSY" });
    }
  ...
  logEvent("info", "assistant_provider_succeeded", { provider: "deepinfra", status: completion.status });
